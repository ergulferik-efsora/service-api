...
package com.epam.reportportal.serviceapi.auth.authenticator;

import static com.epam.reportportal.serviceapi.auth.UserRoleHierarchy.ROLE_REGISTERED;
...
import com.epam.reportportal.serviceapi.auth.acl.ReportPortalAclAuthorizationStrategyImpl;
...
public class RegisteredUserAuthenticator implements UserAuthenticator {
   ...
   * custom implementation {@link ReportPortalAclAuthorizationStrategyImpl} to permit
   * shared objects to the newly created user
   *
   * @param user {@link User}
   * @return {@link Authentication} with authenticated user with the role
   * {@link UserRoleHierarchy#ROLE_REGISTERED}
   */
  @Override
